...
param(
    ...
    [string]$CandidateVersion = '1.1.1-issue33-rc2'
)
...
function Get-GzipPayload([string]$CSharpText,[string]$ConstantName) {
    ...
    try {
        ...
        try {
            ...
            try { $gzip.CopyTo($output); return $output.ToArray() } finally { $output.Dispose() }
...
$workRoot = Join-Path $Base ('ISSUE33_STANDALONE_SKIN_RC2_' + $stamp)
$packageRoot = Join-Path $workRoot 'PACKAGE'
$sourceBuilderFile = Join-Path $workRoot 'SOURCE_FINAL_1_1.ps1'
$patchedBuilder = Join-Path $workRoot 'BUILD_ISSUE33_STANDALONE_SKIN_RC2.ps1'
...
$guiSha = Get-Sha256Bytes $guiBytes; $engineSha = Get-Sha256Bytes $engineBytes
...
$programClassCount = ([regex]::Matches($csharpText,'(?m)^\s*internal\s+static\s+class\s+Program\s*$')).Count
if ($programClassCount -ne 1) { throw ('Expected exactly one internal static class Program declaration, found ' + $programClassCount) }
$staticCtorCount = ([regex]::Matches($csharpText,'(?m)^\s*static\s+Program\s*\(\s*\)\s*$')).Count
if ($staticCtorCount -ne 0) { throw ('Original wrapper already contains static Program() constructor; found ' + $staticCtorCount) }
...
$guiConst = $guiConstMatches[0]; $indent = $guiConst.Groups['indent'].Value
$insertion = @'
private const string Issue33SkinGzipBase64 = @"__ISSUE33_SKIN_GZIP_BASE64__";
private const string Issue33SkinSha256 = "__ISSUE33_SKIN_SHA256__";

static Program()
{
    Issue33EnsureEmbeddedSkin();
}
...
private static void Issue33EnsureEmbeddedSkin()
{
    ...
    if (File.Exists(skinPath))
    {
        ...
        if (String.Equals(existingHash, Issue33SkinSha256, StringComparison.OrdinalIgnoreCase)) return;
    }
...
'@
$insertionLines = ($insertion -split "`r?`n") | ForEach-Object { if ($_.Length -gt 0) { $indent + $_ } else { '' } }
$insertionText = ($insertionLines -join "`r`n") + "`r`n"
$csharpPatched = $csharpText.Insert($guiConst.Index,$insertionText)
if (([regex]::Matches($csharpPatched,'(?m)^\s*static\s+Program\s*\(\s*\)\s*$')).Count -ne 1) { throw 'Patched wrapper does not contain exactly one static Program() constructor.' }
...
Write-Host 'C# bootstrap insertion       : PASS (explicit static Program())' -ForegroundColor Green
...
Test-PowerShellText $builderPatched 'Issue33 RC2 patched builder'
$utf8 = [Text.UTF8Encoding]::new($false)
if ($hadBom) {
    $body = $utf8.GetBytes($builderPatched); $outBytes = New-Object byte[] ($body.Length + 3)
    $outBytes[0]=0xEF; $outBytes[1]=0xBB; $outBytes[2]=0xBF; [Array]::Copy($body,0,$outBytes,3,$body.Length)
    [IO.File]::WriteAllBytes($patchedBuilder,$outBytes)
} else { [IO.File]::WriteAllText($patchedBuilder,$builderPatched,$utf8) }
$patchedBuilderSha = Get-Sha256File $patchedBuilder
Write-Host ('Issue33 RC2 builder SHA256  : ' + $patchedBuilderSha) -ForegroundColor Green

Write-Host ''; Write-Host 'Building non-release standalone-skin RC2 candidate...' -ForegroundColor Cyan
...
foreach ($p in @($candidateExe,$candidateManifest,$candidateRepair,$candidateUi,$candidatePaper)) { if (-not (Test-Path -LiteralPath $p -PathType Leaf)) { throw ('Candidate bundle missing: ' + $p) } }
...
$report = Join-Path $workRoot 'ISSUE33_RC2_BUILD_REPORT.txt'
$lines = @(
'AOTR 8P WOTR ISSUE #33 STANDALONE SKIN RC2',
...
'C# bootstrap: explicit static Program() constructor',
'Pre-launch internal/ absent: PASS',
'Pre-launch root file count = 5: PASS',
'',
'NEXT',
'Run isolated first-launch bootstrap gate. Candidate must materialize internal\assets\launcher_skin.png itself before GUI load.'
)
[IO.File]::WriteAllLines($report,$lines,[Text.UTF8Encoding]::new($false))
Write-Host ''; Write-Host '============================================================' -ForegroundColor Green
Write-Host ' ISSUE #33 RC2 BUILD: PASS' -ForegroundColor Green
...
Write-Host ''; Write-Host 'DO NOT PUBLISH. Next gate is isolated first-launch bootstrap + START smoke.' -ForegroundColor Yellow
