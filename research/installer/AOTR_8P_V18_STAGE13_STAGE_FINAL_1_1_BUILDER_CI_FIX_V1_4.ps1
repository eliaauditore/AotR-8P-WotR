...
$Runtime = Join-Path $env:TEMP 'AOTR_8P_V18_STAGE13_V1_4_RUNTIME.ps1'
...
# Patch 1: validate Config V2 where it actually lives: embedded GUI gzip payload.
$oldConfigCheck = "if (`$text -notmatch 'aotr-standalone-v2') { throw 'Robust autodetect Config V2 marker missing from generated builder.' }"
$newConfigCheck = @'
...
'@
$text = Replace-ExactOnce $text $oldConfigCheck $newConfigCheck 'Config V2 validation layer'

# Patch 2: replace the old guessed LauncherVersion block with real V18 default detection.
$versionBlockPattern = '(?s)# Canonical FINAL_1_1 builder defaults to 1\.1 when invoked without an explicit version\.[\r\n]+\$patchedText = \$text[\r\n]+if \(\$patchedText\.Contains\("\[string\]`\$LauncherVersion = ''1\.0\.10''"\)\) \{.*?[\r\n]+Test-PowerShellText \$patchedText ''canonical FINAL_1_1 builder'''
$versionBlockMatches = [regex]::Matches($text,$versionBlockPattern)
if ($versionBlockMatches.Count -ne 1) { throw ('Expected exactly one old LauncherVersion handling block, found ' + $versionBlockMatches.Count) }
$newVersionBlock = @'
...
$launcherVersionMatches = [regex]::Matches($patchedText,$launcherVersionPattern)
if ($launcherVersionMatches.Count -ne 1) { throw ('Expected exactly one actual V18 LauncherVersion default declaration, found ' + $launcherVersionMatches.Count) }
...
Test-PowerShellText $patchedText 'canonical FINAL_1_1 builder'
'@
$text = [regex]::Replace($text,$versionBlockPattern,$newVersionBlock,1)

[IO.File]::WriteAllText($Runtime,$text,[Text.UTF8Encoding]::new($false))
$tokens=$null;$errors=$null
[void][System.Management.Automation.Language.Parser]::ParseInput($text,[ref]$tokens,[ref]$errors)
if ($errors.Count -gt 0) { $errors | Format-List *; throw 'STOP: Stage13 V1.4 runtime has parser errors.' }

Write-Host 'Stage13 V1.4 parser validation: PASS' -ForegroundColor Green
