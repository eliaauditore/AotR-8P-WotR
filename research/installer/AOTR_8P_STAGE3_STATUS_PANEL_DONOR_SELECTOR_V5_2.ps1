...
function Get-Matches([string]$Text,[string]$Pattern) {
    foreach ($m in [regex]::Matches($Text,$Pattern,[Text.RegularExpressions.RegexOptions]::IgnoreCase)) { $m }
}
...
Write-Host ' AOTR 8P STAGE 3 STATUS PANEL DONOR SELECTOR V5.2' -ForegroundColor Cyan
...
$sortScoreThenFile = @(
    @{ Expression = 'EvidenceScore'; Descending = $true },
...
$rows | Sort-Object -Property $sortScoreThenFile | Format-Table File,Lines,VersionMarkers,DynamicControls,DynamicFunctions,StatusSetFunctions,InvokePreflight,RowFailRefs,ReadyPatchRefs,SuccessWords,EvidenceScore -AutoSize | Out-Host
...
$ordered = @($rows | Sort-Object -Property $sortScoreLinesFile)
...
$report = Join-Path $EvidenceRoot 'STATUS_PANEL_DONOR_SELECTOR_V5_2.txt'
$summary = @()
$summary += 'AOTR 8P STATUS PANEL DONOR SELECTOR V5.2'
$summary += ('Generated: ' + [DateTime]::UtcNow.ToString('o'))
$summary += ''
$summary += ($rows | Sort-Object -Property $sortScoreThenFile | Format-Table File,SHA256,Lines,VersionMarkers,DynamicControls,DynamicFunctions,StatusSetFunctions,InvokePreflight,RowFailRefs,ReadyPatchRefs,SuccessWords,EvidenceScore -AutoSize | Out-String)
if ($top.Count -eq 1) {
...
Write-Host ' DONOR SELECTOR V5.2 COMPLETE' -ForegroundColor Green
Write-Host '============================================================' -ForegroundColor Green
