...
$SourceTemp = Join-Path $env:TEMP 'AOTR_8P_ISSUE33_PR34_STAGER_V1_SOURCE.ps1'
$Runtime = Join-Path $env:TEMP 'AOTR_8P_ISSUE33_PR34_STAGER_V1_1_RUNTIME.ps1'
...
$badCheck = "if (`$builderText -notmatch 'Issue33SkinGzipBase64') { throw 'Final builder no longer contains the embedded skin bootstrap marker.' }"
$replacement = "Write-Host '[PASS] exact FINAL_1_1_1 builder SHA pins the embedded skin bootstrap bytes' -ForegroundColor Green"
$text = Replace-ExactOnce $text $badCheck $replacement 'outer-builder plaintext marker false-fail removal'
...
if (@($errors).Count -gt 0) {
    $errors | Format-List *
    throw 'STOP: Issue33 PR34 stager V1.1 runtime has parser errors.'
}

Write-Host 'Issue33 PR34 stager V1.1 runtime parser validation: PASS' -ForegroundColor Green
Write-Host ('Pinned V1 blob : ' + $ExpectedGitBlobSha1)
Write-Host ('Runtime        : ' + $Runtime)
Write-Host 'Fix            : remove invalid plaintext search inside Base64-encoded C# template' -ForegroundColor Green
Write-Host ''
