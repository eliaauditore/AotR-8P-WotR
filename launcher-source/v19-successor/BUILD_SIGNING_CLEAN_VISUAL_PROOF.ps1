...
param(
    [string]$OutputRoot = "",
    [string]$LauncherVersion = "1.1.3-signingclean-visualproof1"
)
...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $env:TEMP "AOTR8P_SIGNING_CLEAN_VISUAL_PROOF"
}
...
# The six excluded visual resources must exist in the historical donor-free branch so the
# proof can explicitly demonstrate exclusion rather than silently losing an input.
$ExcludedVisuals = @(
    "launcher_skin.png",
    "launcher.ico",
    "row1cleanpatch.png",
...
# Preserve the existing V7 source-chain gate before changing any GUI/C# presentation wiring.
& $V7Verifier -ResourcesRoot $Resources

# Build a neutral, project-controlled WPF presentation from the maintained GUI source.
...
$windowReplacement = @'
        ...
        ShowInTaskbar="True">
'@
$guiText = Replace-ExactOnce $guiText $windowAnchor.TrimStart("`r","`n") $windowReplacement.TrimStart("`r","`n") "neutral main window background"
...
# Prepare the C# host with only the four remaining embedded resources.
$source = [IO.File]::ReadAllText($SourceTemplate).Replace("`r`n","`n")
foreach ($line in @(
    '    private const string SkinResourceName = "AotR8P.LauncherSkin";`n',
    '    private const string Row1PatchResourceName = "AotR8P.Row1Patch";`n',
    '    private const string Row2PatchResourceName = "AotR8P.Row2Patch";`n',
    '    private const string Row3PatchResourceName = "AotR8P.Row3Patch";`n',
    '    private const string ReadyPatchResourceName = "AotR8P.ReadyPatch";`n',
    '                    runspace.SessionStateProxy.SetVariable("AOTR8P_SKIN_BYTES", ReadEmbeddedBytes(SkinResourceName));`n',
    '                    runspace.SessionStateProxy.SetVariable("AOTR8P_ROW1_PATCH", ReadEmbeddedBytes(Row1PatchResourceName));`n',
    '                    runspace.SessionStateProxy.SetVariable("AOTR8P_ROW2_PATCH", ReadEmbeddedBytes(Row2PatchResourceName));`n',
    '                    runspace.SessionStateProxy.SetVariable("AOTR8P_ROW3_PATCH", ReadEmbeddedBytes(Row3PatchResourceName));`n',
    '                    runspace.SessionStateProxy.SetVariable("AOTR8P_READY_PATCH", ReadEmbeddedBytes(ReadyPatchResourceName));`n'
)) {
    $actualLine = $line.Replace('`n',"`n")
    $source = Replace-ExactOnce $source $actualLine "" ("remove C# visual dependency: " + $actualLine.Trim())
}
...
$expectedNames = @("AotR8P.EngineScript","AotR8P.FinalStableV7","AotR8P.GuiScript","AotR8P.V7Shellcode" | Sort-Object)
...
$report = [ordered]@{
    ...
    note = "Research proof only. Binary is intentionally not a field candidate and should not be distributed for local execution."
}
