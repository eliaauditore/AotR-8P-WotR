...
$text = Replace-Once $text '"_V21_1_1_4_BUILD"' '"_V26_1_1_8_RC2_BUILD"' 'default output root'
...
'@
$newTransform = @'
$SourceTemplateV26 = Join-Path $Work "launcher_v26.cs"
& $Python `
    (Join-Path $RepoRoot "launcher-source\v25\apply_issue84_session_runtime.py") `
...
& $Python `
    (Join-Path $RepoRoot "launcher-source\v25\apply_issue84_session_runtime_hardening.py") `
    (Join-Path $Resources "launcher_engine.ps1")
if ($LASTEXITCODE -ne 0) { throw "Issue84 session hardening transform failed" }
& $Python `
    (Join-Path $RepoRoot "launcher-source\v25\apply_issue84_legacy_cleanup_hardening.py") `
    (Join-Path $Resources "launcher_gui.ps1")
if ($LASTEXITCODE -ne 0) { throw "Issue84 chained legacy cleanup hardening failed" }
& $Python `
    (Join-Path $RepoRoot "launcher-source\v26\apply_issue84_session_runtime_rc2_hardening.py") `
    (Join-Path $Resources "launcher_engine.ps1") `
    (Join-Path $Resources "launcher_gui.ps1") `
    $SourceTemplateV26
if ($LASTEXITCODE -ne 0) { throw "Issue84 RC2 lifecycle hardening failed" }
'@
$text = Replace-Once $text $oldTransform $newTransform 'session transform invocation'
$text = Replace-Once $text '$SourceTemplate = Join-Path $RepoRoot "launcher-source\v20\launcher.cs"' '$SourceTemplate = $SourceTemplateV26' 'C# source template redirect'
$text = Replace-Once $text 'AotR_8P_War_of_the_Ring_1.1.4_Issue78_RC1.zip' 'AotR_8P_War_of_the_Ring_1.1.8_Issue84_SESSION_RC2.zip' 'RC zip name'
...
if ($engineText -notmatch '--cleanup-runtime-watch') { throw "Issue84 RC2 lifecycle watcher missing" }
if ($engineText -notmatch [regex]::Escape('runtime\sessions')) { throw "Issue84 local session root missing" }
if ($engineText -notmatch 'Write-AotR8PSessionMarker') { throw "Issue84 marker writer missing" }
if ($engineText -notmatch 'schema = 2') { throw "Issue84 RC2 marker schema missing" }
if ($engineText -notmatch 'launcher_start_utc_ticks') { throw "Issue84 launcher process identity missing" }
if ($engineText -notmatch 'game_launcher_start_utc_ticks') { throw "Issue84 game-launcher process identity missing" }
if ($engineText -notmatch 'game_start_utc_ticks') { throw "Issue84 game process identity missing" }
...
if ($guiText -notmatch '--cleanup-runtime-stale') { throw "Issue84 Auto-Repair stale-only cleanup mode missing" }
if ($guiText -match [regex]::Escape('"--cleanup-runtime",')) { throw "Issue84 unsafe zero-PID cleanup mode remains" }
...
$csText = [IO.File]::ReadAllText($SourceTemplateV26)
foreach ($needle in @(
    ...
    if ($csText -notmatch [regex]::Escape($needle)) { throw "Issue84 RC2 C# contract missing: $needle" }
}
foreach ($forbidden in @('RunRuntimeCleanupHelper','WaitForPidExit','String.Equals(args[0], "--cleanup-runtime",')) {
    if ($csText -match [regex]::Escape($forbidden)) { throw "Issue84 RC1 weak lifecycle primitive remains: $forbidden" }
}
Write-Host "ISSUE84 RC2 LIFECYCLE STATIC CONTRACT PASS" -ForegroundColor Green
'@
$text = Replace-Once $text $oldStatic $newStatic 'Issue84 RC2 static contract insertion'
