...
# Purpose: catch the exact instruction that changes DE892C during a normal client join.
# This script uses an x86 hardware data breakpoint (DR0/DR7) while attached as a debugger.
# It does NOT WriteProcessMemory, VirtualProtect, VirtualAllocEx, or call any game function.
# Thread debug registers are restored/cleared before detach.
#
# IMPORTANT: run under 32-bit Windows PowerShell (SysWOW64) because game.dat is 32-bit.

$ExpectedHash = 'CC08275D60FF8E3BFD4374C29D61304DEA8336E6DD00AB8ADD88B1DF95A705DC'
$TargetAddress = [uint32]0x00DE892C
...
public static class AotrHwWatch32 {
    ...
    static bool ConfigureThread(uint tid, uint target, bool enable, bool suspendFirst) {
        ...
        try {
            ...
            if (enable) {
                c.Dr0 = target;
                // Local enable DR0 + write(01b) + len4(11b).
                c.Dr7 &= ~0x000F0003u;
                c.Dr7 |= 0x000D0001u;
                c.Dr6 = 0;
            } else {
    ...
    public static string Watch(int pid, uint target, int timeoutMs) {
        ...
        uint initial = ReadU32(ph,target);
        log.AppendLine("INITIAL_TARGET="+F(initial));
        log.AppendLine("WATCH_ADDRESS="+F(target));
        ...
        try {
                ...
                if (ev.dwDebugEventCode==EXCEPTION_DEBUG_EVENT) {
                    ...
                    if (code==EXCEPTION_SINGLE_STEP) {
                        ...
                        if (CaptureThread(ev.dwThreadId,out c) && (c.Dr6 & 1u)!=0) {
                            uint now=ReadU32(ph,target);
                            log.AppendLine("WRITE_HIT=YES");
                            ...
                            log.AppendLine("EIP_AFTER="+F(c.Eip));
                            log.AppendLine("TARGET_VALUE="+F(now));
                            ...
                            if (now!=initial) {
                                log.AppendLine("TARGET_CHANGED=YES");
                                ConfigureAllThreads(pid,target,false,false);
                                stop=true;
                            } else {
                                log.AppendLine("TARGET_CHANGED=NO");
                            }
                            c.Dr6=0;
                        }
            ...
            uint finalValue=ReadU32(ph,target);
            log.AppendLine("FINAL_TARGET="+F(finalValue));
            if (finalValue==initial) log.AppendLine("TARGET_CHANGE_OBSERVED=NO");
            else log.AppendLine("TARGET_CHANGE_OBSERVED=YES");
        }
        finally {
            try {
                if (eventPending) {
...
Write-Output '============================================================'
Write-Output ' AOTR WOTR DE892C HARDWARE WRITE WATCH - NO MEMORY PATCH'
Write-Output '============================================================'
Write-Output ("PID          : {0}" -f $ProcessId)
Write-Output ("Image        : {0}" -f $GameDat)
Write-Output ("SHA256       : {0}" -f $hash)
Write-Output ("Watch address: 0x{0:X8}" -f $TargetAddress)
Write-Output ("Timeout      : {0}s" -f $TimeoutSeconds)
Write-Output ''
Write-Output 'Procedure: start in the normal browser, then click normal Join exactly once.'
Write-Output 'The debugger temporarily uses thread DR0/DR7 hardware debug registers; it does not patch game memory.'
Write-Output ''

$result = [AotrHwWatch32]::Watch($ProcessId,$TargetAddress,$TimeoutSeconds*1000)
Write-Output $result
Write-Output 'WATCH COMPLETE. No WriteProcessMemory or game-function call was used.'
