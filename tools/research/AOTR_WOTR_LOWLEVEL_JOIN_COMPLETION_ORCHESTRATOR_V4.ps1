...
function Read-TextSafe {
    ...
    try { return [string](Get-Content -LiteralPath $FilePath -Raw -ErrorAction Stop) }
    catch { return '' }
}
...
function Start-Ps32Hidden {
    ...
    if ($null -eq $ChildArguments -or $ChildArguments.Count -eq 0) {
        throw 'Start-Ps32Hidden received no child arguments.'
    }
    foreach ($item in $ChildArguments) {
        if ($null -eq $item -or [string]::IsNullOrWhiteSpace([string]$item)) {
            throw 'Start-Ps32Hidden received a null/empty child argument.'
        }
    }
    if ([string]::IsNullOrWhiteSpace($StdOutPath) -or [string]::IsNullOrWhiteSpace($StdErrPath)) {
        throw 'Start-Ps32Hidden requires non-empty stdout/stderr paths.'
    }

    return Start-Process -FilePath $ps32 `
...
Write-Host '============================================================'
Write-Host ' AOTR WOTR LOWLEVEL JOIN COMPLETION ORCHESTRATOR V4'
Write-Host '============================================================'
...
# PHASE 0: prove the exact hidden-child argument/redirect mechanism before any game interaction.
...
if (-not $selfProc.WaitForExit(5000)) {
    Stop-ProcSafe -ProcessObject $selfProc
    throw 'TOOLING_SELFTEST_TIMEOUT. No debugger or game call was attempted.'
}
$selfText = Read-TextSafe -FilePath $selfOut
$selfErrText = Read-TextSafe -FilePath $selfErr
if ($selfProc.ExitCode -ne 0 -or $selfText.Trim() -ne 'PS32_CHILD_OK') {
    throw ("TOOLING_SELFTEST_FAILED. No debugger or game call was attempted.`n--- OUT ---`n{0}`n--- ERR ---`n{1}" -f $selfText,$selfErrText)
}
Write-Host 'TOOLING_SELFTEST_PASS' -ForegroundColor Green

# Resolve one clean VM game.dat. Waiting is safe and does not mutate anything.
...
Download-Tool -Name 'AOTR_WOTR_STATE8_COMPLETION_DUAL_EXEC_WATCH_V4.ps1' -Destination $watchPath
...
foreach ($file in @($preOut,$preErr,$readyFile,$stopFile,$statusFile,$watchOut,$watchErr,$joinOut,$joinErr,$combined)) {
    Remove-Item -LiteralPath $file -Force -ErrorAction SilentlyContinue
}

# PHASE 1: dry-run exact join contract before attaching debugger.
Write-Host 'PHASE 1/4 PREFLIGHT - validating pre-join contract...'
$preArgs = @(
    '-NoProfile','-ExecutionPolicy','Bypass',
    '-File',(Quote-Arg $wrapperPath),
    '-SourcePath',(Quote-Arg $joinPath),
    '-ProcessId',[string]$pid32,
    '-ExpectedRemoteIp',(Quote-Arg $ExpectedRemoteIp),
...
if (-not $preProc.WaitForExit(15000)) {
    Stop-ProcSafe -ProcessObject $preProc
    throw 'PREFLIGHT_TIMEOUT. No debugger or join was started.'
}
$preText = Read-TextSafe -FilePath $preOut
$preErrText = Read-TextSafe -FilePath $preErr
if ($preProc.ExitCode -ne 0 -or $preText -notmatch 'CALL_POC_READY = YES') {
...
# PHASE 2: debugger must explicitly signal READY before join can start.
Write-Host 'PHASE 2/4 WATCHER - attaching and arming hidden debugger...'
$watchArgs = @(
    '-NoProfile','-ExecutionPolicy','Bypass',
    '-File',(Quote-Arg $watchPath),
    '-ProcessId',[string]$pid32,
...
if ($armedCount -le 0) {
    ...
    [void]$watchProc.WaitForExit(5000)
...
# PHASE 3: execute exactly one native +0x40 join.
...
if (-not $joinProc.WaitForExit(25000)) {
    ...
    [void]$watchProc.WaitForExit(6000)
    Stop-ProcSafe -ProcessObject $joinProc
    Stop-ProcSafe -ProcessObject $watchProc
...
# PHASE 4: request deterministic debugger teardown and require clean exit.
...
if (-not $watchProc.WaitForExit(10000)) {
    $statusText = Read-TextSafe -FilePath $statusFile
    $watchOutText = Read-TextSafe -FilePath $watchOut
    $watchErrText = Read-TextSafe -FilePath $watchErr
    Stop-ProcSafe -ProcessObject $watchProc
...
$joinText = Read-TextSafe -FilePath $joinOut
$joinErrText = Read-TextSafe -FilePath $joinErr
$watchText = Read-TextSafe -FilePath $watchOut
$watchErrText = Read-TextSafe -FilePath $watchErr
$statusText = Read-TextSafe -FilePath $statusFile
...
$watchDone        = ($statusText -match 'STAGE=DONE') -and ($watchProc.ExitCode -eq 0)
$valid            = $joinReturned -and $joinObserved -and $currentC54 -and $de892cStayedNull -and $watchDone

$lines = New-Object 'System.Collections.Generic.List[string]'
$lines.Add('============================================================')
$lines.Add(' AOTR WOTR LOWLEVEL JOIN COMPLETION - COMBINED RESULT V4')
$lines.Add('============================================================')
...
$lines.Add('TOOLING_SELFTEST_PASS     : YES')
...
$lines.Add($statusText.TrimEnd())
$lines.Add('')
$lines.Add('================ JOIN STDOUT ================')
$lines.Add($joinText.TrimEnd())
if (-not [string]::IsNullOrWhiteSpace($joinErrText)) {
    $lines.Add('')
    $lines.Add('================ JOIN STDERR ================')
    $lines.Add($joinErrText.TrimEnd())
}
$lines.Add('')
$lines.Add('================ WATCHER STDOUT ================')
$lines.Add($watchText.TrimEnd())
if (-not [string]::IsNullOrWhiteSpace($watchErrText)) {
    $lines.Add('')
    $lines.Add('================ WATCHER STDERR ================')
    $lines.Add($watchErrText.TrimEnd())
}
