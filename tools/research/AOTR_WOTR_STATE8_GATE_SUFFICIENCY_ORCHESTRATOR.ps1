...
$WatcherRef     = 'b35d207898ea7e730a8e5d176e8e3e7754f7e923' # exact V5 watcher used by the valid 1708-hit run
...
if ($log -notmatch '(?m)^TEST_VALID_FOR_STATE8\s+:\s+YES\s*$') { throw 'Source log is not marked TEST_VALID_FOR_STATE8=YES.' }
if ($log -notmatch '(?m)^WATCHER_CLEAN_EXIT\s+:\s+YES\s*$') { throw 'Source log is not marked WATCHER_CLEAN_EXIT=YES.' }
if ($log -notmatch '(?m)^JOIN_STATE_OBSERVED\s+:\s+YES\s*$') { throw 'Source log is not marked JOIN_STATE_OBSERVED=YES.' }
if ($log -notmatch '(?m)^CURRENT_C54B78\s+:\s+YES\s*$') { throw 'Source log is not marked CURRENT_C54B78=YES.' }
if ($log -notmatch '(?m)^DE892C_STAYED_NULL\s+:\s+YES\s*$') { throw 'Source log is not marked DE892C_STAYED_NULL=YES.' }
...
if (-not $p) { throw "Original game PID $GamePid is no longer running. This sufficiency test intentionally refuses a different process; create a fresh low-level-joined state first." }
if ($p.ExecutablePath -ine $GameDat) { throw "PID $GamePid path mismatch: $($p.ExecutablePath)" }

if (-not ('AotrState8Mem32' -as [type])) {
Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
public static class AotrState8Mem32 {
...
function Read32([uint32]$Address) { return [uint32][AotrState8Mem32]::Read32($GamePid,$Address) }
function Write32([uint32]$Address,[uint32]$Value) { [AotrState8Mem32]::Write32($GamePid,$Address,$Value) }
...
Write-Host '============================================================'
Write-Host ' AOTR WOTR STATE8 GATE SUFFICIENCY ORCHESTRATOR'
Write-Host '============================================================'
...
Write-Host 'Forbidden writes : DE892C, session+0x44, GameInfo rows (none are written)'
Write-Host ''

# Prove the exact already-runtime-tested watcher still loads under the VM's 32-bit CLR.
...
try {
    ...
    # Re-check after watcher attach/arm, immediately before the single controlled mutation.
    ...
    $verify=Read32 $stateAddress
    if($verify -ne 8){throw "STATE_WRITE_VERIFY_FAILED: read back $verify"}
    $stateWritten=$true
    Write-Host 'STATE8_WRITE_VERIFIED owner+0x6A4: 1 -> 8' -ForegroundColor Yellow
    ...
    while([DateTime]::UtcNow -lt $deadline){
        ...
        if($s.Net -ne 0 -and $s.Net -eq $s.Current){$publicationObserved=$true;break}
    }

    $afterObserve=Get-LiveState
    # Failure-safety only: if nothing published and the owner is still stuck at our injected 8,
    # restore the single DWORD before asking the proven watcher to detach.
    ...
    $sufficiency=$stateWritten -and $callbackHit -and $completionHit -and $published -and $watchClean

    $lines=New-Object System.Collections.Generic.List[string]
    $lines.Add('============================================================');$lines.Add(' AOTR WOTR STATE8 GATE SUFFICIENCY RESULT');$lines.Add('============================================================')
    ...
    $lines.Add('================ VERDICT ================')
    $lines.Add('PREWRITE_CONTRACT_PASS    : YES');$lines.Add(("STATE8_WRITE_VERIFIED      : {0}" -f $(if($stateWritten){'YES'}else{'NO'})));$lines.Add(("CALLBACK_8496C2_HIT        : {0}" -f $(if($callbackHit){'YES'}else{'NO'})));$lines.Add(("COMPLETION_84944F_HIT      : {0}" -f $(if($completionHit){'YES'}else{'NO'})));$lines.Add(("DE892C_EQUALS_CURRENT      : {0}" -f $(if($published){'YES'}else{'NO'})));$lines.Add(("WATCHER_CLEAN_EXIT         : {0}" -f $(if($watchClean){'YES'}else{'NO'})));$lines.Add(("STATE8_SUFFICIENCY_PROVEN  : {0}" -f $(if($sufficiency){'YES'}else{'NO'})));$lines.Add(("FAILURE_STATE_RESTORED     : {0}" -f $(if($stateRestored){'YES'}else{'NO'})));$lines.Add('')
    ...
    if($stateWritten -and -not $cleanStop){
        try{
            $s=Get-LiveState
            if($s.Net -eq 0 -and $s.State -eq 8){Write32 ([uint32]($Owner+0x6A4)) 1; $stateRestored=$true}
        }catch{}
    }
