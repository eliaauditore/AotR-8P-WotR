...
# Validates the exact native C54CE0 +0x40 join-call contract against a live game.dat process.
# Enumerates the session GameInfo list from session+0x10 and checks which objects are safe
# arg1 candidates for 0x84CB34. No WriteProcessMemory / VirtualAllocEx / CreateRemoteThread.
...
if (-not ('A8PJoinRuntimeRead' -as [type])) {
Add-Type -TypeDefinition @"
using System;
using System.ComponentModel;
using System.Runtime.InteropServices;

public static class A8PJoinRuntimeRead
{
...
function Read-Bytes([int64]$addr,[int]$count) {
    [A8PJoinRuntimeRead]::Read([uint32]$ProcessId,$addr,$count)
}
...
function Format-IPv4BE([uint32]$raw) {
    ('{0}.{1}.{2}.{3}' -f (($raw -shr 24)-band 0xFF),(($raw -shr 16)-band 0xFF),(($raw -shr 8)-band 0xFF),($raw-band 0xFF))
}
...
function Safe-U32([int64]$addr,[ref]$ok) {
    try { $ok.Value=$true; return (Read-U32 $addr) } catch { $ok.Value=$false; return [uint32]0 }
}
...
Write-Host '============================================================'
Write-Host ' AOTR WOTR NATIVE JOIN RUNTIME PREFLIGHT - READ ONLY'
Write-Host '============================================================'
...
while ($p -ne 0 -and $index -lt 32) {
    ...
    $mark = if ($p -eq $current44) {' CURRENT'} elseif ($p -eq $network) {' NETWORK'} else {''}
    ...
    $candidates += [pscustomobject]@{
        Index=$index; Ptr=$p; VtableOk=$vtOk; Row0=$row0; Row0Type=$row0Type;
        Ip=$row0Ip; Port=$row0Port; Arg1Safe=$arg1Safe; RemoteCandidate=$remoteJoinCandidate;
        Current=($p -eq $current44); Network=($p -eq $network)
    }

    $p = [uint32]$next
    $index++
}
if ($index -ge 32 -and $p -ne 0) { Write-Host 'List truncated at 32 nodes.' -ForegroundColor Yellow }
if ($candidates.Count -eq 0) { Write-Host '<no GameInfo objects in session+0x10 list>' }
Write-Host ''
...
Write-Host 'arg2 convention      : native caller may pass pointer to {0,0}; 0x84C257 treats it as no explicit endpoint.'
...
if ($ready) {
    ...
    Write-Host 'BRIDGE_CALL_READY = NO' -ForegroundColor Yellow
    if (-not $sessionOk) { Write-Host 'Reason: session vtable mismatch.' }
    if (-not $stateOk) { Write-Host ("Reason: session+0x28={0}; native +0x40 only enters ID3 build path for state 0 or 2." -f $state28) }
    if ($arg1SafeCandidates.Count -eq 0) { Write-Host 'Reason: no validator-compatible GameInfo with readable row0.' }
    elseif ($remoteCandidates.Count -eq 0) { Write-Host 'Reason: GameInfo objects exist, but none has a non-local row0 endpoint; no safe remote join target selected/discovered yet.' }
}
