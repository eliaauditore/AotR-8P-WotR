...
# V5 bootstrap for the controlled State8-before-native-join experiment.
# It preserves V4's RF-safe V6 watcher and exact reviewed V2 orchestration body.
# V4 produced all three explicit CompileOnly PASS markers with empty STDERR but
# still rejected the child because powershell.exe reported a nonzero ExitCode.
# V5 therefore gates CompileOnly on the explicit semantic PASS markers plus empty
# STDERR after a confirmed WaitForExit(), while retaining the child ExitCode as
# diagnostic evidence only. No game process is touched before those gates pass.
...
$tempV2 = Join-Path $env:TEMP ("AOTR_STATE8_PREJOIN_V5_BODY_$stamp.ps1")
...
try {
    Write-Host '============================================================'
    Write-Host ' AOTR STATE8 PREJOIN V5 - RF MARKER-GATED BOOTSTRAP'
    Write-Host '============================================================'
    ...
    $layoutPass   = [bool]($cot -match 'CLR_LAYOUT_SELFTEST_PASS DEBUG_EVENT_SIZE=96 HTHREAD_OFFSET=12 EXADDR_OFFSET=24')
    $rfPass       = [bool]($cot -match 'HW_RESUME_RF_SELFTEST_PASS DR6=0 RF=YES EFLAGS=0x[0-9A-Fa-f]{8}')
    $completePass = [bool]($cot -match 'COMPILE_ONLY_COMPLETE - no game process was opened or debugged\.')
    $stderrPass   = [bool]([string]::IsNullOrWhiteSpace($cet))
    $exitCodeDiag = $null
    try { $exitCodeDiag = $p.ExitCode } catch { $exitCodeDiag = 'UNAVAILABLE' }

    Write-Host ("V6_COMPILEONLY_MARKERS layout={0} rf={1} complete={2} stderrEmpty={3} exitCodeDiagnostic={4}" -f $layoutPass,$rfPass,$completePass,$stderrPass,$exitCodeDiag)

    if (-not $layoutPass -or -not $rfPass -or -not $completePass -or -not $stderrPass) {
        throw ("V6_COMPILEONLY_FAILED. NOTHING EXECUTED.`nOUT:`n{0}`nERR:`n{1}" -f $cot,$cet)
    }
    if ($exitCodeDiag -is [int] -and $exitCodeDiag -ne 0) {
        Write-Host ("V6_COMPILEONLY_EXITCODE_DIAGNOSTIC_NONZERO={0}; semantic PASS markers + empty STDERR are authoritative." -f $exitCodeDiag) -ForegroundColor Yellow
    }
