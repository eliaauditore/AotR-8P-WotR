...
# V6 keeps V5's dual hardware execution-breakpoint design, but fixes resume
# semantics for x86 execution breakpoints. On a DR0/DR1 hit the stopped EIP is
# the instruction being watched. Before ContinueDebugEvent V6 clears DR6 and
# sets EFLAGS.RF so that instruction is allowed to execute once rather than
# being eligible to retrigger at the same EIP indefinitely.
#
# The watcher does not patch game-memory bytes and calls no game function.
...
if (-not ('AotrDualExec32V6RF' -as [type])) {
...
public static class AotrDualExec32V6RF {
    ...
    const uint SG=0x00DE4394, NG=0x00DE892C, UI=0x00DEA114, OG=0x00DE8D90, C54=0x00C54B78;
    ...
    static void MarkHardwareResume(ref CTX c){ c.Dr6=0; c.EFlags|=RF; }
    public static string ResumePolicySelfTest(){
        CTX c=New(CAP); c.Dr6=3; c.EFlags=0x202u; MarkHardwareResume(ref c);
        return "DR6="+c.Dr6+" RF="+(((c.EFlags&RF)!=0)?"YES":"NO")+" EFLAGS=0x"+c.EFlags.ToString("X8");
    }
    ...
    static bool AckHardwareHit(uint tid,out uint before,out uint after){
        before=0; after=0;
        IntPtr h=OpenThread(TA,false,tid); if(h==IntPtr.Zero)return false;
        try{
            CTX c=New(CD|CC); if(!GetThreadContext(h,ref c))return false;
            before=c.EFlags; MarkHardwareResume(ref c); after=c.EFlags;
            return SetThreadContext(h,ref c);
    ...
    static void State(StringBuilder l,IntPtr ph,CTX c){
        uint owner=R(ph,OG), os=owner==0?0:R(ph,owner+0x6A4), o304=owner==0?0:R(ph,owner+0x304);
        uint s=R(ph,SG), st=s==0?0:R(ph,s+0x28), cur=s==0?0:R(ph,s+0x44), vt=cur==0?0:R(ph,cur), net=R(ph,NG), ui=R(ph,UI);
        l.AppendLine("SESSION="+F(s)+" STATE28="+st+" CURRENT="+F(cur)+" CURRENT_VT="+F(vt));
        l.AppendLine("DE892C="+F(net)+" DEA114="+ui);
        l.AppendLine("FRONTEND_OWNER_GLOBAL="+F(owner)+" OWNER_6A4="+os+" OWNER_304="+o304+" CURRENT_IS_C54B78="+(vt==C54?"YES":"NO"));
        l.AppendLine("RAW_ECX="+F(c.Ecx)+" RAW_ESI="+F(c.Esi));
    }
    ...
    public static string Watch(int pid,uint cb,uint comp,int timeoutMs,string readyFile,string stopFile,string statusFile){
        ...
        bool attached=false,pending=false,armed=false,stopRequested=false,done=false,cleanDetach=false,processExited=false; DE e=new DE(); Stopwatch sw=Stopwatch.StartNew(); int cbHits=0,compHits=0,newThreads=0,rfAcks=0;
        try{
                    ...
                    if(code==SS){
                        ...
                        if(Get(e.tid,out c)){
                            ...
                            if(h0||h1){
                                ...
                                l.AppendLine("EAX="+F(c.Eax)+" EBX="+F(c.Ebx)+" ECX="+F(c.Ecx)+" EDX="+F(c.Edx)+" ESI="+F(c.Esi)+" EDI="+F(c.Edi)+" EFLAGS="+F(c.EFlags));
                                State(l,ph,c);
                                uint bef,aft;
                                if(!AckHardwareHit(e.tid,out bef,out aft))throw new Exception("Failed to clear DR6/set EFLAGS.RF for tid="+e.tid+" win32="+Marshal.GetLastWin32Error());
                                rfAcks++;
                                l.AppendLine("HW_RESUME_RF_SET=YES EFLAGS_BEFORE="+F(bef)+" EFLAGS_AFTER="+F(aft));
                            }
                        }
                    }
            ...
            l.AppendLine("HW_RESUME_RF_ACK_COUNT="+rfAcks);
...
$layout = [AotrDualExec32V6RF]::LayoutSelfTest()
$expectedLayout = 'DEBUG_EVENT_SIZE=96 HTHREAD_OFFSET=12 EXADDR_OFFSET=24'
if ($layout -ne $expectedLayout) { throw "CLR DEBUG_EVENT layout mismatch. Expected [$expectedLayout], got [$layout]" }
Write-Output ("CLR_LAYOUT_SELFTEST_PASS {0}" -f $layout)

$resume = [AotrDualExec32V6RF]::ResumePolicySelfTest()
if ($resume -notmatch '^DR6=0 RF=YES EFLAGS=0x[0-9A-F]{8}$') { throw "HW resume policy selftest failed: $resume" }
Write-Output ("HW_RESUME_RF_SELFTEST_PASS {0}" -f $resume)
...
Write-Output '============================================================'
Write-Output ' AOTR WOTR STATE8 COMPLETION DUAL EXEC WATCH V6 RF'
Write-Output '============================================================'
...
Write-Output 'RF resume policy: ENABLED for DR0/DR1 hits'
Write-Output ''
$result=[AotrDualExec32V6RF]::Watch($ProcessId,$Callback,$Completion,$TimeoutSeconds*1000,$ReadyFile,$StopFile,$StatusFile)
Write-Output $result
