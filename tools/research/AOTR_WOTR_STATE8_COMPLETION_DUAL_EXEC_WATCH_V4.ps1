    ...
    public static string Watch(int pid,uint cb,uint comp,int timeoutMs,string readyFile,string stopFile,string statusFile){
        ...
        try{
            ...
            while(!done){
                ...
                if(!armed && (e.code==CP||e.code==CT)){ int count=All(pid,cb,comp,true); armed=true; l.AppendLine("ARMED_THREADS="+count); Signal(readyFile,"STATUS=READY\r\nARMED_THREADS="+count+"\r\nPID="+pid+"\r\n"); Signal(statusFile,"STAGE=ARMED\r\nARMED_THREADS="+count+"\r\n"); }
                else if(armed && e.code==CT){ SetBp(e.tid,cb,comp,true); }
