...
try {
    ...
    # 2) Give the filtered trace extra headroom.
    ...
    # 3) Extend BuildStub contract with the absolute manager-global VA.
    ...
    # 4) Filter in the injected x86 stub before any record is written:
...
'@
    ...
    # 5) Resolve the new skip branches to the existing common epilogue immediately before popad.
...
'@
    ...
    # 6) Pass the runtime manager-global VA into the stub builder.
    $oldCall = '$stub = [A8PInputTraceNative]::BuildStub($remoteBase,[uint32]$continue,[uint32]$DataCapacity)'
    $newCall = '$stub = [A8PInputTraceNative]::BuildStub($remoteBase,[uint32]$continue,[uint32]$DataCapacity,[uint32]($base + $MgrGlobalRva))'
    if (-not $text.Contains($oldCall)) { throw 'BuildStub invocation anchor not found.' }
    $text = $text.Replace($oldCall,$newCall)

    # 7) Make V2 identity explicit in the console so stale output is obvious.
    ...
    Write-Host ("V2 source transform  : PID tokens={0}; localRoot in-stub filter=ON; buffer=32 MiB" -f $pidCount)
