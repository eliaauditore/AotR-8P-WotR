...
# V2 adds a deterministic sidecar readiness signal. The C# debugger writes ReadyFile
# immediately after DR0/DR1 have been armed on the existing game threads. This avoids
# relying on redirected stdout, which is only flushed after Watch() returns.
...
if ([Environment]::Is64BitProcess) {
    throw 'Run under C:\Windows\SysWOW64\WindowsPowerShell\v1.0\powershell.exe'
}
...
if (-not [string]::IsNullOrWhiteSpace($ReadyFile)) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($readyDir)) {
        New-Item -ItemType Directory -Path $readyDir -Force | Out-Null
    }
    Remove-Item -LiteralPath $ReadyFile -Force -ErrorAction SilentlyContinue
}

if (-not ('AotrDualExec32V2' -as [type])) {
...
public static class AotrDualExec32V2 {
    ...
    [StructLayout(LayoutKind.Sequential)] public struct ER {
        public uint Code,Flags,Rec,Addr,N;
        [MarshalAs(UnmanagedType.ByValArray, SizeConst=15)] public uint[] Info;
    }
    ...
    static CTX New(uint f) {
        CTX c=new CTX(); c.Flags=f; c.fs.r=new byte[80]; c.x=new byte[512]; return c;
    }
    static uint R(IntPtr h,uint a) {
        if(a==0) return 0;
        byte[] b=new byte[4]; UIntPtr g;
        if(!ReadProcessMemory(h,new IntPtr(unchecked((int)a)),b,new UIntPtr(4u),out g) || g.ToUInt64()!=4) return 0;
        return BitConverter.ToUInt32(b,0);
    }
    static bool Set(uint tid,uint a0,uint a1,bool on,bool susp) {
        IntPtr h=OpenThread(TA,false,tid); if(h==IntPtr.Zero) return false;
        bool s=false;
        try {
            if(susp) { if(SuspendThread(h)==0xffffffff) return false; s=true; }
            CTX c=New(CD); if(!GetThreadContext(h,ref c)) return false;
            if(on) {
                c.Dr0=a0; c.Dr1=a1;
                c.Dr7 &= ~0x000F00FFu;
                c.Dr7 |= 0x00000005u; // local enable DR0 + DR1, execution breakpoints
                c.Dr6=0;
            } else {
                c.Dr0=c.Dr1=0; c.Dr7 &= ~0x000F00FFu; c.Dr6=0;
            }
    ...
    static int All(int pid,uint a0,uint a1,bool on,bool susp) {
        int n=0;
        try {
            foreach(ProcessThread t in Process.GetProcessById(pid).Threads)
                if(Set((uint)t.Id,a0,a1,on,susp)) n++;
        } catch {}
        return n;
    }
    static bool Get(uint tid,out CTX c) {
        c=New(CAP); IntPtr h=OpenThread(TA,false,tid); if(h==IntPtr.Zero) return false;
        try { return GetThreadContext(h,ref c); } finally { CloseHandle(h); }
    }
    static void Clear6(uint tid) {
        IntPtr h=OpenThread(TA,false,tid); if(h==IntPtr.Zero) return;
        try { CTX c=New(CD); if(GetThreadContext(h,ref c)){c.Dr6=0;SetThreadContext(h,ref c);} }
        finally { CloseHandle(h); }
    }
    static string F(uint v){return "0x"+v.ToString("X8");}
    static void State(StringBuilder l,IntPtr ph,CTX c) {
        uint s=R(ph,SG), st=s==0?0:R(ph,s+0x28), cur=s==0?0:R(ph,s+0x44), vt=cur==0?0:R(ph,cur);
        uint net=R(ph,NG), ui=R(ph,UI), os=c.Ecx==0?0:R(ph,c.Ecx+0x6A4), o304=c.Ecx==0?0:R(ph,c.Ecx+0x304);
        l.AppendLine("SESSION="+F(s)+" STATE28="+st+" CURRENT="+F(cur)+" CURRENT_VT="+F(vt));
        l.AppendLine("DE892C="+F(net)+" DEA114="+ui);
        l.AppendLine("ECX_OWNER="+F(c.Ecx)+" OWNER_6A4="+os+" OWNER_304="+o304+" CURRENT_IS_C54B78="+(vt==C54?"YES":"NO"));
    }
    static void Signal(string path,string text) {
        if(String.IsNullOrEmpty(path)) return;
        try { File.WriteAllText(path,text,Encoding.ASCII); } catch {}
    }

    public static string Watch(int pid,uint cb,uint comp,int ms,string readyFile) {
        StringBuilder l=new StringBuilder();
        IntPtr ph=OpenProcess(PR|PQ,false,(uint)pid);
        if(ph==IntPtr.Zero) throw new Exception("OpenProcess failed "+Marshal.GetLastWin32Error());
        bool at=false, pend=false, armed=false; DE e=new DE(); Stopwatch sw=Stopwatch.StartNew();
        int cbHits=0, compHits=0;
        try {
            ...
            while(sw.ElapsedMilliseconds<ms) {
                ...
                if(!armed && (e.code==CP || e.code==CT)) {
                    int count=All(pid,cb,comp,true,false);
                    l.AppendLine("ARMED_THREADS="+count);
                    armed=true;
                    Signal(readyFile,"STATUS=READY\r\nARMED_THREADS="+count+"\r\nPID="+pid+"\r\n");
                } else if(armed && e.code==CT) {
                    Set(e.tid,cb,comp,true,false);
                }

                uint con=CONT; bool stop=false;
                if(e.code==EX) {
                    ...
                    if(code==SS) {
                        ...
                        if(Get(e.tid,out c)) {
                            ...
                            if(h0||h1) {
                                ...
                                State(l,ph,c);
                                Clear6(e.tid);
                                if(h1){All(pid,cb,comp,false,false);stop=true;}
                            }
                        }
                    } else if(code==BP) con=CONT;
                    else con=NA;
                }

                if(!ContinueDebugEvent(e.pid,e.tid,con)) throw new Exception("ContinueDebugEvent failed "+Marshal.GetLastWin32Error());
                pend=false;
                if(stop) break;
                if(e.code==XP){l.AppendLine("PROCESS_EXITED=YES");break;}
            }

            if(!armed) Signal(readyFile,"STATUS=FAIL\r\nSTAGE=ARM\r\nARMED_THREADS=0\r\n");
            if(cbHits==0) l.AppendLine("CALLBACK_8496C2_HIT=NO");
            if(compHits==0) l.AppendLine("COMPLETION_84944F_HIT=NO");
            CTX z=New(CAP); State(l,ph,z);
        }
        finally {
            try {
                if(pend){All(pid,cb,comp,false,false);ContinueDebugEvent(e.pid,e.tid,CONT);}
                else if(at) All(pid,cb,comp,false,true);
            } catch {}
...
Write-Output '============================================================'
Write-Output ' AOTR WOTR STATE8 COMPLETION DUAL EXEC WATCH V2'
Write-Output '============================================================'
...
Write-Output ("Ready file      : {0}" -f $(if([string]::IsNullOrWhiteSpace($ReadyFile)){'<none>'}else{$ReadyFile}))
Write-Output ''

$result = [AotrDualExec32V2]::Watch($ProcessId,$Callback,$Completion,$TimeoutSeconds*1000,$ReadyFile)
Write-Output $result
