...
# Goal: during LOW-LEVEL native join PoC, prove whether the async callback fires,
# what frontendOwner+0x6A4 contains at that instant, and whether it reaches 0x84944F.
# Run under 32-bit Windows PowerShell (SysWOW64).
...
if($ProcessId-eq0){
 ...
 if($m.Count-ne1){throw "Expected exactly one game.dat at $GameDat, found $($m.Count)"}
...
if(-not('AotrDualExec32'-as[type])){
Add-Type -TypeDefinition @'
using System;
using System.Diagnostics;
using System.Runtime.InteropServices;
using System.Text;
public static class AotrDualExec32 {
 const uint EX=1,CT=2,CP=3,XP=5,SS=0x80000004,BP=0x80000003,CONT=0x00010002,NA=0x80010001;
 ...
 static bool Set(uint tid,uint a0,uint a1,bool on,bool susp){IntPtr h=OpenThread(TA,false,tid);if(h==IntPtr.Zero)return false;bool s=false;try{if(susp){if(SuspendThread(h)==0xffffffff)return false;s=true;}CTX c=New(CD);if(!GetThreadContext(h,ref c))return false;if(on){c.Dr0=a0;c.Dr1=a1;c.Dr7&=~0x000F00FFu;c.Dr7|=0x00000005u;c.Dr6=0;}else{c.Dr0=c.Dr1=0;c.Dr7&=~0x000F00FFu;c.Dr6=0;}return SetThreadContext(h,ref c);}finally{if(s)ResumeThread(h);CloseHandle(h);}}
 static int All(int pid,uint a0,uint a1,bool on,bool susp){int n=0;try{foreach(ProcessThread t in Process.GetProcessById(pid).Threads)if(Set((uint)t.Id,a0,a1,on,susp))n++;}catch{}return n;}
 ...
 public static string Watch(int pid,uint cb,uint comp,int ms){StringBuilder l=new StringBuilder();IntPtr ph=OpenProcess(PR|PQ,false,(uint)pid);if(ph==IntPtr.Zero)throw new Exception("OpenProcess failed "+Marshal.GetLastWin32Error());bool at=false,pend=false,armed=false;DE e=new DE();Stopwatch sw=Stopwatch.StartNew();int cbHits=0,compHits=0;try{DebugSetProcessKillOnExit(false);if(!DebugActiveProcess((uint)pid))throw new Exception("DebugActiveProcess failed "+Marshal.GetLastWin32Error());at=true;l.AppendLine("DEBUG_ATTACH=OK");while(sw.ElapsedMilliseconds<ms){if(!WaitForDebugEvent(out e,50))continue;pend=true;if(!armed&&(e.code==CP||e.code==CT)){l.AppendLine("ARMED_THREADS="+All(pid,cb,comp,true,false));armed=true;}else if(armed&&e.code==CT)Set(e.tid,cb,comp,true,false);uint con=CONT;bool stop=false;if(e.code==EX){uint code=e.u.ex.er.Code;if(code==SS){CTX c;if(Get(e.tid,out c)){bool h0=(c.Dr6&1)!=0,h1=(c.Dr6&2)!=0;if(h0||h1){if(h0){cbHits++;l.AppendLine("CALLBACK_8496C2_HIT=YES");l.AppendLine("CALLBACK_HIT_COUNT="+cbHits);}if(h1){compHits++;l.AppendLine("COMPLETION_84944F_HIT=YES");l.AppendLine("COMPLETION_HIT_COUNT="+compHits);}l.AppendLine("ELAPSED_MS="+sw.Elapsed.TotalMilliseconds.ToString("F3",System.Globalization.CultureInfo.InvariantCulture));l.AppendLine("THREAD_ID="+e.tid+" EXCEPTION_ADDRESS="+F(e.u.ex.er.Addr)+" EIP="+F(c.Eip));l.AppendLine("EAX="+F(c.Eax)+" EBX="+F(c.Ebx)+" ECX="+F(c.Ecx)+" EDX="+F(c.Edx)+" ESI="+F(c.Esi)+" EDI="+F(c.Edi));State(l,ph,c);Clear6(e.tid);if(h1){All(pid,cb,comp,false,false);stop=true;}}}}else if(code==BP)con=CONT;else con=NA;}if(!ContinueDebugEvent(e.pid,e.tid,con))throw new Exception("ContinueDebugEvent failed "+Marshal.GetLastWin32Error());pend=false;if(stop)break;if(e.code==XP){l.AppendLine("PROCESS_EXITED=YES");break;}}if(cbHits==0)l.AppendLine("CALLBACK_8496C2_HIT=NO");if(compHits==0)l.AppendLine("COMPLETION_84944F_HIT=NO");CTX z=New(CAP);State(l,ph,z);}finally{try{if(pend){All(pid,cb,comp,false,false);ContinueDebugEvent(e.pid,e.tid,CONT);}else if(at)All(pid,cb,comp,false,true);}catch{}if(at)try{DebugActiveProcessStop((uint)pid);}catch{}if(ph!=IntPtr.Zero)CloseHandle(ph);}return l.ToString();}
}
'@
}
Write-Output '============================================================'
Write-Output ' AOTR WOTR STATE8 COMPLETION DUAL EXEC WATCH'
Write-Output '============================================================'
...
Write-Output ''
Write-Output 'Start in normal browser. While watcher runs, invoke LOW-LEVEL native join PoC from a second console. Do NOT click normal Join.'
Write-Output ''
Write-Output ([AotrDualExec32]::Watch($ProcessId,$Callback,$Completion,$TimeoutSeconds*1000))
