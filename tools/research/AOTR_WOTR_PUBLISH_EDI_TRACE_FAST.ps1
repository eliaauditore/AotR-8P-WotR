...
# First choice: llvm-objdump already on PATH. Do NOT recursively search Visual Studio.
$llvm = Get-Command llvm-objdump.exe -ErrorAction SilentlyContinue
if ($llvm) {
    $llvmPath = $llvm.Source
    ...
    $out | Where-Object { $_ -match '\bedi\b' } | ForEach-Object { $_ }
...
if (-not $py) {
    throw 'No llvm-objdump on PATH and no Python found.'
}

$pyPath = $py.Source
& $pyPath -c "import capstone" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host 'llvm-objdump is not on PATH. Python exists, but capstone is missing.' -ForegroundColor Yellow
    Write-Host ''
    Write-Host ('Install once with:  & "{0}" -m pip install --user capstone' -f $pyPath) -ForegroundColor Yellow
    Write-Host 'Then rerun this script. It only disassembles about 640 bytes.' -ForegroundColor Yellow
    exit 2
}
